...
int findAllKeys(Map<String, Map<String, int>> distances) {
  ...
  var queue = PriorityQueue<num, State>((state) => state.pathLen)
    ..add(State("@", 0, 0));
  while (queue.isNotEmpty) {
    var state = queue.removeFirst();
    if (state.keys == allKeys) return state.pathLen;
...
int parFindAllKeys(Map<String, Map<String, int>> distances) {
  ...
  var queue = PriorityQueue<num, ParState>((state) => state.pathLen)
    ..add(ParState(["0", "1", "2", "3"], 0, 0));
  while (queue.isNotEmpty) {
    var state = queue.removeFirst();
    if (state.keys == allKeys) return state.pathLen;
    int index = state.index;
